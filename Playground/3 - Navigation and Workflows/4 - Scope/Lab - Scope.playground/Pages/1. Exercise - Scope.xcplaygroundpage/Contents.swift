...
// The value of scope is only limited to the for loop as it was initialised there. Initialisation of variable limits the scope of any object confined to the particular function/class/struct/loop/conditional.
...
// Explanation:
// Both print statements compile because 'x' is defined outside the loop, in a wider scope (like the function or global scope).
// This makes it accessible inside the loop, where it's modified, and AFTER the loop, where it's printed again.
//
// In contrast, if you define a variable INSIDE a narrower scope like inside a guard or if-let,
// that variable is ONLY available within that block** and can't be accessed outside.
...
let myCar = Car(make: "Mercedes-Benz", model: "S-Class", year: 2024)
