...
// The function will always return a value
if let age = checkAge("19"){
...
func priceOfItem(item: String) -> Double? {
    if let quantityInStock = stock[item], quantityInStock > 0 {
        return prices[item]
    }
    return nil
}
...
if let price = priceOfItem(item: testItem) , price > 0{
    print("The value of \(testItem) is \(price)")
}
/*:
