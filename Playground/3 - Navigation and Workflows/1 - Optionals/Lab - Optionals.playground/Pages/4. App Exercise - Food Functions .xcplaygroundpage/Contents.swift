...
var testMeal: String = "Breakfast"
if let breakfastMeal = getMealLog(testMeal) {
    var foodItems = [String]()
    for foodItem in breakfastMeal.food {
        foodItems.append(foodItem)
    }
    print("The meal for \(testMeal) is \(foodItems.joined(separator: ", ")) and calories: \(breakfastMeal.calories)")
}else {
    print("No meal available for \(testMeal)")
}
/*:
 ...
 */
func mealLogExist(forMeal : String) -> [String: Any]? {
    if let savedMealLog = UserDefaults.standard.dictionary(forKey: forMeal) {
        return savedMealLog
    }else {
        return nil
    }
...
if let actualMeal = mealLogExist(forMeal: testMeal){
    print("The meal is \(actualMeal)")
} else {
    print("No meal available")
}

print(UserDefaults.standard.dictionary(forKey: "mealLog"))
/*:
