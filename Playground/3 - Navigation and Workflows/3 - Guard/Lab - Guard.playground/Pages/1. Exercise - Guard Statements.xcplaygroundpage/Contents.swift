 ...
 */
func CalculateAreaOfRectangle(x: Double, y: Double) -> Double? {
    guard x > 0 && y > 0 else {
        return nil
    }
    return x * y
...
CalculateAreaOfRectangle(x: 25.2, y: 23)
CalculateAreaOfRectangle(x: 25.2, y: -3)
...
lastNameTextField.text = "The Ripper"
...
if let user = createUser() {
    ...
    print("Failed to create user. Some fields were missing.")
}
